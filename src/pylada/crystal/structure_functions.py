...
class Sq():
    ...
    def strc_func_tot_AL(self):
        ...
        if hasattr(self,'tot_sq_AL'):
            return
        
        # Get the partials first
        if not hasattr('part_sq_AL'):
    ...
    def strc_func_tot_FZ_from_AL(self):
        ...
        if hasattr(self,'tot_sq_FZ_from_AL'):
            return
        
        if not hasattr(self,'part_sq_AL'):
            self.strc_func_part_AL()
    ...
    def strc_func_tot_FZ(self):
        ...
        if hasattr(self,'tot_sq_FZ'):
            return

        from toolbox.atomic_scattering_params import ff, bb

        if not hasattr(self,'part_sq_FZ'):
...
class PDF():
    ...
    def pdf_part(self):
        ...
        # Setting the atribute
        self.g_r_part = g_r_dict
    ...
    def pdf_tot(self):
        """
        Function to compute the total pair distribution functions (PDF) from
        the partials
        """

        if hasattr(self,'g_r_tot'):
            return
        
        if not hasattr(self,'g_r_part'):
            self.pdf_part()
            gr = self.g_r_part
        ...
        xx=self.rs
        yy=np.zeros(np.shape(xx),dtype=float)
        
        for key,ggrr in gr.items():
            ...
            yy = yy + conc0*conc1 * ggrr[:,1]

        self.g_r_tot = np.array(list(zip(xx,yy)))
