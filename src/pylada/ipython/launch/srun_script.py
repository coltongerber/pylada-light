...
""" Runs one job from the jobfolder using srun. 

    This script is called by srun within a SLURM allocation.
"""
...
def main():
    ...
    parser = ArgumentParser(prog="runsrun", description=re.sub("\\s+", " ", __doc__[1:]))
    ...
    parser.add_argument('--timeout', dest="timeout", default=30, type=int,
                        help="Time to wait for job-dictionary to become available "
                             "before timing out (in seconds). A negative or null "
                             "value implies forever. Defaults to 30s.")
    ...
    if isinstance(tstPgm, str) and tstPgm.lower() == 'none':
        tstPgm = None
    ...
    # We just set the communicator to None if testValidProgram is set
    ...
    print(('  ipy/lau/srun_script: jobfolder: %s' % jobfolder))
    print(('  ipy/lau/srun_script: options: %s' % options))
    
    for name in options.names:
        logger.info('ipy/lau/srun_script: testValidProgram: %s' % testValidProgram)
        logger.info('ipy/lau/srun_script: name: %s' % name)
        logger.info('ipy/lau/srun_script: jobfolder[name]: %s' % jobfolder[name])
        logger.info('ipy/lau/srun_script: type(jobfolder[name]): %s' %
                         type(jobfolder[name]))
        logger.info(
            'ipy/lau/srun_script: jobfolder[name].compute: %s' % jobfolder[name].compute)
        logger.info(
            'ipy/lau/srun_script: type(jobfolder[name].compute): %s' \
            % type(jobfolder[name].compute))
        logger.info('ipy/lau/srun_script: before compute for name: %s' % name)

        comm = pylada.default_comm
        if testValidProgram is not None:
            comm = None
        jobfolder[name].compute(comm=comm, outdir=name)
        logger.info('ipy/lau/srun_script: after compute for name: %s' % name)
